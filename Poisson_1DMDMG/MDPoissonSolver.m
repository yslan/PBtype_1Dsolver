function [Xdomain,v] = MDPoissonSolver(Xendpt,DegDM)
...
F = zeros(Nmaxp,TotNumDM);  Mass=F; Jac=F;
Xdomain = F;

D = zeros([Nmaxp Nmaxp TotNumDM]); 
Ahat=D; BlockL=D; SubMat=D; SupMat=D; Im=D; Ip=D;
    
%Mass = zeros([Nmaxp,TotNumDM]); Jac = Mass;
...
for k = 1 : TotNumDM
    ...
    Mass(1:NDp,k) = w;  D(1:NDp,1:NDp,k) = collocD(-x); 
end
...
for k=1:TotNumDM
    NDp=DegDM(k)+1;
    Ahat(1:NDp,1:NDp,k) = diag(A(1:NDp,k)./Jac(1:NDp,k));
end
...
% Compute right hans side
    k = 1; NDp = DegDM(k)+1;  
    F(1:NDp,k) = F(1:NDp,k) + Mass(1:NDp,k) .* sigma_m(1:NDp) * gl;
    
    k = TotNumDM; NDp = DegDM(k)+1; 
    F(1:NDp,k) = F(1:NDp,k) + Mass(1:NDp,k) .* sigma_p(1:NDp) * gr;

...
m=3;
cheb=0;
dtheta=pi/m; theta=dtheta*([1:m] - 1/2); xi=cos(theta');
sigma = 1 + .5*(xi+1); sigma=1./sigma;
if cheb==0; sigma=.66666 + 0*sigma; end;

for v=1:10;                    % 10 V-cycles
  for j=1:m                    % Apply smoother m times
    u=u+sigma(j)*(D\r); r=f-A*u;
  ...
  u  = u + ef;
  ip = plot_err(u,ex,z,V,L,'r-',v,0,ispec);  %pause

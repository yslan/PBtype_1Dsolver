...
debye_D = sqrt(epsilon*k_B*T/(2*c0*e^2*z^2));
vv = 2*rad^3*c0;

tmp1 = epsilon/debye * abs(sinh(z*e*V0/(k_B*T)));
tmp2 = 1+2*vv*sinh(e*z*V0/(2*k_B*T)).^2;
tmp3 = sqrt(2/vv*log(tmp2));

ex_C_D = tmp1./(tmp2.*tmp3);
